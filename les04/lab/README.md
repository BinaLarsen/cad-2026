...
1. Результат лабораторной работы № 1 скопирован в каталог `les04/lab/` (модуль Gradle `app`, пакет `ru.bsuedu.cad.lab`).
...
7. Приложение запускается командой `./gradlew run` из каталога `les04/lab`.
...
```bash
cp ../demo/spring-aop/gradle/wrapper/gradle-wrapper.jar gradle/wrapper/
chmod +x gradlew
```
